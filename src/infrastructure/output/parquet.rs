//! Parquet row-group export.
use super::{ExportStats, Exporter, ensure_parent_dir, f32_ms_to_i64, strip_ip_prefix};
...
impl ColumnBuffer {
    fn with_capacity(field_index: usize, capacity: usize) -> Self {
        match field_index {
            ...
            _ => Self::Utf8(StringBuilder::with_capacity(
                capacity,
...
impl ParquetExporter {
    ...
    fn append_record(&mut self, sqllog: &LogRecord, normalized: Option<&str>) {
        ...
        for (column, &idx) in self.columns.iter_mut().zip(&self.ordered_indices) {
            match (column, idx) {
                (ColumnBuffer::Utf8(v), 0) => v.append_value(&sqllog.ts),
                (ColumnBuffer::Int32(v), 1) => v.push(Some(i32::from(sqllog.ep))),
                (ColumnBuffer::Utf8(v), 2) => v.append_value(&sqllog.sess_id),
                (ColumnBuffer::Utf8(v), 3) => v.append_value(&sqllog.thrd_id),
                (ColumnBuffer::Utf8(v), 4) => v.append_value(&sqllog.username),
                (ColumnBuffer::Utf8(v), 5) => v.append_value(&sqllog.trxid),
                (ColumnBuffer::Utf8(v), 6) => v.append_value(&sqllog.statement),
                (ColumnBuffer::Utf8(v), 7) => v.append_value(&sqllog.appname),
                (ColumnBuffer::Utf8(v), 8) => {
                    v.append_value(strip_ip_prefix(&sqllog.client_ip));
                }
                (ColumnBuffer::Utf8(v), 9) => v.append_option(sqllog.tag.as_deref()),
                (ColumnBuffer::Utf8(v), 10) => v.append_value(&sqllog.sql),
...
#[path = "../../../tests/unit/infrastructure/output/parquet.rs"]
mod tests;
