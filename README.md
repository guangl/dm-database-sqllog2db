...
Input .log files (sqllogs/)
    v SqllogParser         -- discovers files
    v dm-database-parser-sqllog  -- parses each line into Sqllog records
    v Pipeline             -- optional filters and template analysis
    v ExporterManager      -- routes to active exporter
    v Output (CSV / SQLite)
```
