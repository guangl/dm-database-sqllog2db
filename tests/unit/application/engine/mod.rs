use super::run as handle_run;
use crate::config::Config;
use std::sync::Arc;
use std::sync::atomic::AtomicBool;

/// 构造一个用于记录级单元测试的 `RunContext`：空 pipeline、全字段掩码。
fn make_test_ctx(cfg: &Config, do_normalize: bool) -> super::context::RunContext<'_> {
    super::context::RunContext {
        cfg,
        pipeline: crate::pipeline::Pipeline::new(),
        do_normalize,
        normalize_tags: vec!["SEL".to_string()],
...
mod collector {
    ...
    /// 收集到的记录与其可选归一化 SQL。
    type CollectedRows = Vec<(LogRecord, Option<String>)>;
...
fn test_two_group_transaction_filters_across_files() {
    ...
    for (source, expected) in cases {
        ...
        for path in &files {
            for record in crate::input::open_log_file(path).unwrap() {
...
#[test]
fn csv_and_parquet_share_file_progress_and_single_output() {
    use super::context::{Console, build_run_context};
    use super::sequential::run_sequential;
    for format in ["csv", "parquet"] {
        let dir = tempfile::tempdir().unwrap();
        let files: Vec<_> = ["a.log", "b.log"].map(|name| dir.path().join(name)).into();
        for file in &files {
            std::fs::write(file, "2025-01-15 10:30:28.001 (EP[0] sess:0x1 user:U trxid:1 stmt:0x1 appname:A ip:10.0.0.1) [SEL] SELECT 1. EXECTIME: 1(ms) ROWCOUNT: 1(rows) EXEC_ID: 1.\n").unwrap();
        }
        ...
        .unwrap();
        let progress = super::prepare::make_progress_bar(true, files.len()).unwrap();
        let console = Console {
            quiet: false,
            verbose: true,
            pb: Some(&progress),
        };
        let (counts, stats) = run_sequential(
            &build_run_context(&cfg),
            &files,
            &console,
            &Arc::new(AtomicBool::new(false)),
        )
        .unwrap();
        assert_eq!(progress.position(), 2, "{format}");
