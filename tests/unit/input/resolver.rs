...
fn test_log_files_nonexistent_path() {
    let p = InputResolver::new(vec!["/this/does/not/exist/at/all".to_string()]);
...
fn test_log_files_single_file() {
    ...
    let p = InputResolver::new(vec![file_path.to_string_lossy().into_owned()]);
    let files = p.log_files().unwrap();
    assert_eq!(files.len(), 1);
...
fn test_log_files_invalid_glob_pattern() {
    ...
    let p = InputResolver::new(vec!["/tmp/[invalid".to_string()]);
...
fn test_log_files_multi_input_merge_and_dedup() {
    ...
    // 重复 dir_a：dedup 应过滤掉
    let p = InputResolver::new(vec![input_a.clone(), input_b, input_a]);
    let files = p.log_files().unwrap();
    assert_eq!(
        files.len(),
        2,
