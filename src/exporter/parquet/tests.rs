...
fn sample_record() -> dm_database_parser_sqllog::Sqllog {
    let mut source = tempfile::NamedTempFile::new().unwrap();
    writeln!(
        source,
        "2025-01-15 10:30:28.001 (EP[0] sess:0x0001 user:TESTUSER trxid:1 stmt:0x1 appname:App ip:10.0.0.1) [SEL] SELECT 1. EXECTIME: 0(ms) ROWCOUNT: 0(rows) EXEC_ID: 0."
    )
    .unwrap();
    LogParserBuilder::new(source.path())
        .build()
        .unwrap()
        .iter()
        .unwrap()
