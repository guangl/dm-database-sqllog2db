...
#[test]
fn test_export_fatal_is_critical() {
    let err = Error::Export(ExportError::Fatal {
        reason: "locked".into(),
    });
    assert!(err.is_fatal(), "ExportError::Fatal should be fatal");
    assert_eq!(
        err.severity(),
        ErrorSeverity::Critical,
        "ExportError::Fatal should have Critical severity"
    );
    assert!(
        err.suggestion().contains("output"),
        "ExportError::Fatal suggestion should mention output, got: {}",
        err.suggestion()
    );
}
