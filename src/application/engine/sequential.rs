use crate::engine::context::{Console, FileCounts, RunContext};
...
pub(crate) fn run_sequential(
    ...
    let loop_result = run_file_loop(ctx, log_files, &mut exporter_manager, console, interrupted);
...
fn run_file_loop(
    ...
    for (idx, log_file) in log_files.iter().enumerate() {
        ...
        if file_stats.has_fatal() {
            return Err(Error::Export(crate::error::ExportError::Fatal {
